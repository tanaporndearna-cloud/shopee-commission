...
with col1:
    erp_file = st.file_uploader("Data ERP.xlsx *", type=["xlsx"])
    shopee_files = st.file_uploader("Data SHOPEE (หลายไฟล์ได้)", type=["xlsx"], accept_multiple_files=True)

with col2:
    abbhsp_files = st.file_uploader("ABBHSP (หลายไฟล์ได้)", type=["xlsx"], accept_multiple_files=True)
    fin_files = st.file_uploader("การเงิน SHOPEE (หลายไฟล์ได้)", type=["xlsx"], accept_multiple_files=True)
        ...
        try:
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                if stock_override:
                    ...
                    with open(os.path.join(stock_dir, "STOCK.xlsx"), "wb") as f:
                    ...
                    if stock_csv_path:
                        import pandas as pd
                        log(f"✅ STOCK: {len(pd.read_csv(stock_csv_path)):,} rows")
                    else:
                        log("⚠️ ไม่มี STOCK")
                ...
                for sf in shopee_files:
                    with open(os.path.join(shopee_dir, sf.name), "wb") as f:
                        f.write(sf.getvalue())
                if shopee_files: log(f"✅ Data SHOPEE: {len(shopee_files)} ไฟล์")
                ...
                for af in abbhsp_files:
                    with open(os.path.join(abbhsp_dir, af.name), "wb") as f:
                        f.write(af.getvalue())
                if abbhsp_files: log(f"✅ ABBHSP: {len(abbhsp_files)} ไฟล์")
                ...
                for ff in fin_files:
                    with open(os.path.join(fin_dir, ff.name), "wb") as f:
                        f.write(ff.getvalue())
                if fin_files: log(f"✅ การเงิน SHOPEE: {len(fin_files)} ไฟล์")
                ...
                for line in buf.getvalue().splitlines(): log(f"  {line}")
                ...
                log(f"\n⏳ อัปโหลด {output_filename} ไปยัง Google Drive...")
                try:
                    ...
                    st.warning(f"อัปโหลดไป Drive ไม่ได้ ({e})")
